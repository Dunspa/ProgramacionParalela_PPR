...
extern int rank;  // Identificador del proceso dentro de cada comunicador
                  // (coincide en ambos)
...
int color_token;  // Color del token la �ltima vez que estaba en poder del
                  // proceso
...
bool difundir_cs_local;  // Indica si el proceso puede difundir su cota inferior
                         // local
...
bool Inconsistente(int **tsp) {
    ...
    for (fila = 0; fila < NCIUDADES; fila++) { /* examina cada fila */
        ...
        for (i = 0, n_infinitos = 0; i < NCIUDADES; i++)
            if (tsp[fila][i] == INFINITO && i != fila) n_infinitos++;
        if (n_infinitos == NCIUDADES - 1) return true;
    }
...
void Reduce(int **tsp, int *ci) {
    ...
    for (v = 0; v < NCIUDADES; v++) {
        for (w = 0, min = INFINITO; w < NCIUDADES; w++)
            if (tsp[v][w] < min && v != w) min = tsp[v][w];
        if (min != 0) {
            for (w = 0; w < NCIUDADES; w++)
                if (tsp[v][w] != INFINITO && v != w) tsp[v][w] -= min;
...
void IncluyeArco(tNodo *nodo, tArco arco) {
    ...
    if (nodo->orig_excl() == arco.v) {
        int i;
        nodo->datos[1]++;
        for (i = 0; i < NCIUDADES - 2; i++) nodo->dest_excl()[i] = NULO;
    }
}

bool ExcluyeArco(tNodo *nodo, tArco arco) {
    int i;
    if (nodo->orig_excl() != arco.v) return false;
...
void PonArco(int **tsp, tArco arco) {
    int j;
    for (j = 0; j < NCIUDADES; j++) {
        if (j != arco.w) tsp[arco.v][j] = INFINITO;
        if (j != arco.v) tsp[j][arco.w] = INFINITO;
    }
}
...
void ApuntaArcos(tNodo *nodo, int **tsp) {
    ...
    for (arco.v = 0; arco.v < NCIUDADES; arco.v++)
        if ((arco.w = nodo->incl()[arco.v]) != NULO) PonArco(tsp, arco);
    for (arco.v = nodo->orig_excl(), i = 0; i < NCIUDADES - 2; i++)
        if ((arco.w = nodo->dest_excl()[i]) != NULO) QuitaArco(tsp, arco);
...
void InfiereArcos(tNodo *nodo, int **tsp) {
    ...
    do {
        ...
        for (i = 0; i < NCIUDADES; i++) /* para cada fila i */
            if (nodo->incl()[i] ==
                NULO) { /* si no hay incluido un arco <i,?> */
...
void Reconstruye(tNodo *nodo, int **tsp0, int **tsp) {
    int i, j;
    for (i = 0; i < NCIUDADES; i++)
        for (j = 0; j < NCIUDADES; j++) tsp[i][j] = tsp0[i][j];
...
void HijoIzq(tNodo *nodo, tNodo *lnodo, int **tsp, tArco arco) {
    ...
    for (i = 0; i < NCIUDADES; i++)
        for (j = 0; j < NCIUDADES; j++) tsp2[i][j] = tsp[i][j];
...
bool Solucion(tNodo *nodo) {
    int i;
    for (i = 0; i < NCIUDADES; i++)
        if (nodo->incl()[i] == NULO) return false;
    return true;
}
...
int Tamanio(tNodo *nodo) {
    ...
    for (i = 0, cont = 0; i < NCIUDADES; i++)
        if (nodo->incl()[i] == NULO) cont++;
    return cont;
}

void InicNodo(tNodo *nodo) {
    nodo->datos[0] = nodo->datos[1] = 0;
    for (int i = 2; i < 2 * NCIUDADES; i++) nodo->datos[i] = NULO;
}
...
void EscribeNodo(tNodo *nodo) {
    ...
    printf("ci=%d : ", nodo->ci());
    for (i = 0; i < NCIUDADES; i++)
        if (nodo->incl()[i] != NULO) printf("<%d,%d> ", i, nodo->incl()[i]);
...
bool tPila::push(tNodo &nodo) {
    ...
    for (int i = 0; i < 2 * NCIUDADES; i++) nodos[tope + i] = nodo.datos[i];
...
bool tPila::pop(tNodo &nodo) {
    ...
    for (int i = 0; i < 2 * NCIUDADES; i++) nodo.datos[i] = nodos[tope + i];

    return true;
}

bool tPila::divide(tPila &pila2) {
    if (vacia() || tamanio() == 1) return false;
