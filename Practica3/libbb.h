...
class tNodo {
   public:
    ...
    tNodo() { datos = new int[2 * NCIUDADES]; }

    int ci() { return datos[0]; }

    int orig_excl() { return datos[1]; }

    int *incl() { return &datos[2]; }

    int *dest_excl() { return &datos[NCIUDADES + 2]; }
    ~tNodo() { delete[] datos; }
};
#endif
...
class tPila {
   public:
    ...
    inline bool llena() const { return (tope == 2 * NCIUDADES * MAXPILA); }

    inline bool vacia() const { return (tope == 0); }

    inline int tamanio() const { return (tope / (2 * NCIUDADES)); }
